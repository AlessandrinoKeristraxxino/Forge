...
function builtinHover(chain, modules) {
    // Normalize chain:
    ...
    for (const key of candidates) {
        ...
        if (doc.doc)
            lines.push(doc.doc);
        if (doc.module) {
            const enabled = modules.allInOneEnabled || modules.enabled.has(doc.module);
            lines.push("");
...
/* =========================================================
   ...
   ========================================================= */
function extractWordAt(src, offset) {
    if (offset < 0 || offset > src.length)
        return null;
    // Expand around offset for [A-Za-z0-9_]
    let s = offset;
    let e = offset;
    while (s > 0 && isWordChar(src.charCodeAt(s - 1)))
        s--;
    while (e < src.length && isWordChar(src.charCodeAt(e)))
        e++;
    if (e <= s)
        return null;
    return src.slice(s, e);
}
function extractMemberChainAt(src, offset) {
    ...
    let s = offset;
    let e = offset;
    while (s > 0 && isChainChar(src.charCodeAt(s - 1)))
        s--;
    while (e < src.length && isChainChar(src.charCodeAt(e)))
        e++;
    const chunk = src.slice(s, e);
    // Validate chain shape: starts with ident, can have ".ident"
    if (!/^[A-Za-z_][A-Za-z0-9_]*(\.[A-Za-z_][A-Za-z0-9_]*)*$/.test(chunk))
        return null;
    return chunk;
}
