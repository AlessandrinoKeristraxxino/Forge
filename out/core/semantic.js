...
function analyzeProgram(program, opts = {}) {
...
const T = {
    ...
    object(props = {}, open = true) {
        return { kind: "object", props, open };
    },
    func(params, returns, isAsync = false) {
        return { kind: "function", params, returns, isAsync };
    },
};
function isAssignableTo(a, b) {
    ...
    if (a.kind === b.kind) {
        if (a.kind === "array")
            return isAssignableTo(a.element, b.element);
        if (a.kind === "object") {
            const ob = b;
            for (const [k, tv] of Object.entries(ob.props)) {
                const av = a.props?.[k] ?? T.unknown();
                if (!isAssignableTo(av, tv))
                    return false;
            }
            return true;
        }
        if (a.kind === "function") {
            ...
            for (let i = 0; i < fa.params.length; i++) {
                if (!isAssignableTo(fa.params[i], fb.params[i]))
                    return false;
            }
...
function unify(a, b) {
    ...
    if (a.kind === b.kind) {
        if (a.kind === "array")
            return T.array(unify(a.element, b.element));
        if (a.kind === "object") {
            const pa = a.props ?? {};
            const pb = b.props ?? {};
            const out = {};
            ...
            for (const k of keys)
                out[k] = unify(pa[k] ?? T.unknown(), pb[k] ?? T.unknown());
            return T.object(out, a.open || b.open);
        }
        if (a.kind === "function") {
            const fa = a;
            const fb = b;
            ...
            const params = [];
            for (let i = 0; i < n; i++)
                params.push(unify(fa.params[i], fb.params[i]));
...
function defaultModules() {
    ...
    return {
        ...
        enabled: new Set(["AllInOne"]),
    };
}
function fn(name, params, returns, module) {
    return { name, params, returns, module };
}
function val(name, type, module) {
...
class SemanticAnalyzer {
    ...
    analyze(program) {
        ...
        for (const st of program.body ?? []) {
            if (st?.kind === "DisableDirective")
                this.applyDisable(st);
            if (st?.kind === "AbleDirective")
                this.applyAble(st);
        }
    ...
    /* =========================================================
       ...
       ========================================================= */
    applyDisable(node) {
        const target = this.readStringValue(node.target);
        if (!target)
            return;
        if (target === "AllInOne") {
    ...
    visitStatement(node) {
        ...
        switch (node.kind) {
            ...
            case "ExpressionStatement":
                ...
                return;
            case "BlockStatement":
                this.withBlockScope(() => this.visitBlock(node));
                return;
            case "IfStatement":
                this.visitIf(node);
                return;
            ...
            default:
                ...
                this.warn(node.range ?? fakeRange0(), `Unknown statement kind '${node.kind}'.`, "SEM_UNKNOWN_STMT");
                return;
        }
    }
    visitBlock(node) {
    ...
    visitVarDeclaration(node) {
        const declKind = node.declKind;
        const name = node.name?.name ?? node.name?.value ?? "";
        const r = node.range ?? node.name?.range ?? fakeRange0();
        if (!name) {
        ...
        if (node.initializer)
            initType = this.visitExpression(node.initializer);
        const sym = {
            name,
            declaredAt: node.name?.range ?? r,
            mutability: declKind,
            type: initType,
        ...
        if (declKind === "var") {
            ...
            if (!ok)
                this.error(sym.declaredAt, `Variable '${name}' is already declared in this scope.`, "SEM_REDECL");
            return;
        }
    ...
    visitAssignmentStatement(node) {
        ...
        if (tType && !isAssignableTo(vType, tType)) {
            this.warn(node.range, `Type mismatch: cannot assign ${fmtType(vType)} to ${fmtType(tType)}.`, "SEM_TYPE_ASSIGN");
        }
    ...
    visitIf(node) {
        ...
        for (const e of elifs) {
            const et = this.visitExpression(e.test);
            if (et.kind !== "boolean" && et.kind !== "unknown" && et.kind !== "any") {
                this.warn(e.test.range, `Condition should be boolean, got ${fmtType(et)}.`, "SEM_COND_BOOL");
            }
            this.withBlockScope(() => this.visitStatement(e.consequent));
        }
        if (alt)
            this.withBlockScope(() => this.visitStatement(alt));
    }
    visitWhile(node) {
        const test = node.test;
    ...
    visitForEach(node) {
        this.withBlockScope(() => {
            ...
            if (name) {
                ...
                const ok = this.scope.define(sym);
                if (!ok)
                    this.error(sym.declaredAt, `Loop variable '${name}' is already declared in this scope.`, "SEM_REDECL");
            }
        ...
        if (handler) {
            this.withBlockScope(() => {
                ...
                if (param?.name) {
                    ...
                    if (!ok)
                        this.error(param.range, `Catch parameter '${param.name}' is already declared.`, "SEM_REDECL");
                }
                this.visitStatement(handler.body);
            });
        }
        if (finalizer)
            this.withBlockScope(() => this.visitStatement(finalizer));
    }
    visitFunctionDeclaration(node) {
        // Define function symbol in current scope
        const name = node.name?.name ?? "";
        if (name) {
            const fnType = T.func((node.params ?? []).map(() => T.unknown()), T.unknown(), !!node.isAsync);
            const ok = this.scope.define({
                name,
                declaredAt: node.name?.range ?? node.range,
                mutability: "const",
                type: fnType,
                store: this.scope.kind,
            });
            if (!ok)
                this.error(node.name?.range ?? node.range, `Function '${name}' is already declared.`, "SEM_REDECL");
        }
        // Analyze body in function scope
        this.withFunctionScope(() => {
            this.defineParams(node.params ?? []);
    ...
    visitExpression(node) {
        ...
        switch (node.kind) {
            ...
            case "NamespacedIdentifier":
                return this.visitNamespacedIdentifier(node);
            ...
            case "BooleanOpExpression":
                return this.visitBooleanOp(node);
            ...
            case "TemplateString":
                ...
                return T.string();
            default:
                this.warn(node.range, `Unknown expression kind '${node.kind}'.`, "SEM_UNKNOWN_EXPR");
    ...
    visitNamespacedIdentifier(node) {
    ...
    visitMemberExpression(node) {
        const objT = this.visitExpression(node.object);
    ...
    visitCallExpression(node) {
        ...
        if (callSig) {
            ...
            for (let i = 0; i < count; i++) {
                if (!isAssignableTo(argTypes[i], expected[i])) {
                    this.warn(args[i]?.range ?? node.range, `Argument ${i + 1} expects ${fmtType(expected[i])}, got ${fmtType(argTypes[i])}.`, "SEM_ARG_TYPE");
                }
            }
        ...
        if (calleeT.kind === "function") {
            const ret = calleeT.returns ?? T.unknown();
        ...
        if (op === "+" || op === "-" || op === "x" || op === "/" || op === "%" || op === "§") {
            ...
            if (l.kind === "duration" || r.kind === "duration") {
                if (op === "+" || op === "-") {
                    ...
                    if ((l.kind === "duration" && (r.kind === "duration" || r.kind === "number" || r.kind === "unknown" || r.kind === "any")) ||
            ...
            // numeric
            if ((l.kind !== "number" && l.kind !== "unknown" && l.kind !== "any") ||
    ...
    visitBooleanOp(node) {
        ...
        const subT = this.visitExpression(node.subject);
        // query/cast => boolean
        void subT;
        this.setType(node, T.boolean());
        return T.boolean();
    ...
    visitAwait(node) {
        ...
        this.setType(node, argT.kind === "function" ? argT.returns ?? T.unknown() : T.unknown());
        return T.unknown();
    }
    ...
    visitFunctionExpression(node) {
        const params = (node.params ?? []).map(() => T.unknown());
    ...
    /* =========================================================
       ...
       ========================================================= */
    resolveAssignableType(target, write) {
        if (!target)
            return null;
        if (target.kind === "Identifier") {
            ...
            if (!sym) {
                ...
                if (write)
                    this.error(target.range, `Cannot assign to undeclared variable '${name}'.`, "SEM_ASSIGN_UNDECL");
                return T.unknown();
            }
            return sym.type;
        }
        ...
        if (target.kind === "MemberExpression") {
            ...
            if (objT.kind === "object") {
                ...
                if (write && !objT.props[key])
                    objT.props[key] = T.unknown();
                return current;
            }
            return T.unknown();
        }
    ...
    resolveStore(ns, name) {
        if (ns === "l")
            return this.storeL.get(name) ?? null;
        if (ns === "v")
            return this.storeV.get(name) ?? null;
        return this.storeC.get(name) ?? null;
    }
    defineStore(ns, name, mutability, declaredAt, type) {
        const sym = {
            name,
            declaredAt,
    ...
    resolveCallSignature(callee) {
        ...
        const attemptCall = (members, parts) => {
            let cur = members;
            for (const p of parts) {
                const next = cur?.[p];
                if (!next)
                    return null;
                if (isBuiltinNamespace(next))
                    cur = next.members;
                else if (isBuiltinFn(next))
                    cur = next; // leaf can be fn
                else if (isBuiltinValue(next))
                    cur = next; // leaf is value
            }
            if (isBuiltinFn(cur))
                return cur;
            if (isBuiltinNamespace(cur)) {
                const call = cur.members["__call__"];
                if (call && isBuiltinFn(call))
                    return call;
            }
            return null;
        };
        // Walk root
        const root = path[0];
        const rootNode = BUILTIN_APIS.members[root];
        if (!rootNode)
            return null;
        ...
        if (isBuiltinNamespace(rootNode)) {
            ...
            if (call && isBuiltinFn(call) && path.length === 1)
                return call;
            return null;
        }
        // root itself is fn or value (rare)
        if (isBuiltinFn(rootNode) && path.length === 1)
            return rootNode;
        return null;
    }
    checkModuleGatingForCall(node) {
        if (this.opts.ignoreModuleGating)
            return;
        if (this.modules.allInOneEnabled)
            return;
        const callee = node.callee;
        const path = this.getCalleePath(callee);
        const root = path[0];
        ...
        const need = (reqFromSig ?? required);
        if (isModuleRoot(root) || reqFromSig) {
            if (!this.modules.enabled.has(need)) {
                this.error(node.range, `Module '${need}' is not enabled. Add: able '${need}' (or remove disable 'AllInOne').`, "SEM_MODULE_DISABLED");
            }
        }
    }
    checkModuleGatingForMember(node) {
        if (this.opts.ignoreModuleGating)
            return;
        if (this.modules.allInOneEnabled)
            return;
    ...
    getCalleePath(expr) {
        ...
        if (expr.kind === "MemberExpression")
            return this.getMemberPath(expr);
        return [];
    }
    getMemberPath(expr) {
        const parts = [];
    ...
    findFunctionOrGlobalScope() {
        let s = this.scope;
        while (s) {
            if (s.kind === "function" || s.kind === "global")
                return s;
    ...
    defineParams(params) {
        for (const p of params) {
            ...
            if (!ok)
                this.error(p?.name?.range ?? p?.range ?? fakeRange0(), `Parameter '${name}' is already declared.`, "SEM_REDECL");
        }
    }
    /* =========================================================
       Type tracking
    ...
    makeIndex() {
        const global = {};
        for (const s of this.globalScope.allLocal())
            global[s.name] = s;
        const l = {};
        const v = {};
        const c = {};
        for (const [k, s] of this.storeL.entries())
            l[k] = s;
        for (const [k, s] of this.storeV.entries())
            v[k] = s;
        for (const [k, s] of this.storeC.entries())
            c[k] = s;
        return { global, l, v, c };
    }
    /* =========================================================
